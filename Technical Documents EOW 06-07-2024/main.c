...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define ID						0x0202	//ID of controller
...
uint16_t LastLED = 0;               // Stores the last known illuminated LED
GPIO_TypeDef* LastGPIO = NULL;      // Stores the last known GPIO for LED illumination
...
int main(void)
{
  ...
  while (1)
  {
	...
	{
		if(LastLED != 0)						//If a Floor LED has been turned on
		{
			HAL_GPIO_TogglePin(LastGPIO, LastLED);	//Turn off last known Floor LED
		}
		HAL_Delay(2000);
		for(i=0; i<8; i++)
		{
		...
		HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_9);	//Floor 1 LED On
		HAL_Delay(100);
		LastLED = GPIO_PIN_9;					//Store Floor 1 LED Pin
		LastGPIO = GPIOC;						//Store Respective GPIO
	}
	...
	if (RxData[0] == GO_TO_FLOOR_2)			//If Rx Floor 2 Button Pressed Toggle Floor 1 LED's
	{
		...
		HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_8);	//Floor 2 LED On
		HAL_Delay(100);
		LastLED = GPIO_PIN_8;					//Store Floor 2 LED Pin
		LastGPIO = GPIOB;						//Store Respective GPIO
	}
	...
	if (RxData[0] == GO_TO_FLOOR_3)			//If Rx Floor 3 Button Pressed Toggle Floor 1 LED's
	{
		...
		HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_9);	//Floor 3 LED On
		HAL_Delay(100);
		LastLED = GPIO_PIN_9;					//Store Floor 2 LED Pin
		LastGPIO = GPIOB;						//Store Respective GPIO
	}
	...
	if (BUTTON != 0)
	{
		...
		if(BUTTON == GO_TO_FLOOR_3)				//If Floor 3 is Requested Tx Toggle All Nodes LED
		{
			...
			HAL_GPIO_TogglePin(GPIOC, GPIO_PIN_5);	//Floor 3 Button LED
			HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_9);	//Floor 3 LED On
			LastLED = GPIO_PIN_9;					//Store Floor 3 LED Pin
			LastGPIO = GPIOB;						//Store Respective GPIO
			BUTTON = NO_BUTTON_PRESSED;
		}
	}
